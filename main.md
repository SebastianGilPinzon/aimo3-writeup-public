...
**Final result:** public 42/50, private 39.5/50; final rank 2049 / 3452 teams.
**Submission:** [AIMO3 v7 - Winner Fork - Version 12](https://www.kaggle.com/code/sebastiangil00/aimo3-v7-winner-fork)
**Base model (HuggingFace):** [unsloth/gpt-oss-120b](https://huggingface.co/unsloth/gpt-oss-120b) (MXFP4, unmodified — no fine-tuning, no LoRA merge)
**Kaggle-agnostic reproduction:** [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/SebastianGilPinzon/aimo3-writeup-public/blob/v1.0-rebuttal/colab_inference.ipynb) — one-click, CPU-tier compatible for the rule-based detectors; H100 path for full pipeline.
**Reproducibility package:** [GitHub `v1.0-rebuttal`](https://github.com/SebastianGilPinzon/aimo3-writeup-public/tree/v1.0-rebuttal) (frozen tag; unit tests + `reproduce.sh` + figure-generator + `reproducibility/COMMIT.txt` with the pinned commit SHA) · [Kaggle Dataset](https://www.kaggle.com/datasets/sebastiangil00/aimo3-writeup-artifacts)

**Score basis:** public 42/50, private 39.5/50 (−2.5 pp gap = 0.4σ inside the variance-model band of §2.7), rank 2049 / 3452 teams (final private leaderboard, 2026-05).

---

## Abstract

We ship three artifacts for future AIMO entrants on the **gpt-oss-120b-MXFP4 + vLLM 0.11.2** substrate: runnable detectors for three named substrate traps, a sixteen-row falsification catalog, and a one-command Kaggle-agnostic reproduction of our final 42/50 submission. The pinned substrate is `gpt-oss-120b` [1, 2] (MoE expert projections in MXFP4, attention and router in BF16) served through the vLLM 0.11.2 wheel on a single NVIDIA H100 80GB GPU.
...
- **LoRA-on-MXFP4 merge-then-requantize collapse** (§6.2, `tests/test_lora_mxfp4_collapse.py`) — the AIMO3 manifestation of the adapter–quantization incompatibility addressed by QA-LoRA [14]. Merging a published AIMO2 LoRA adapter [4] into the MXFP4-quantized base and re-quantizing produces `W′ ≈ W`: the low-magnitude delta `BA` rounds into the same E2M1 [1] representable bin as the base weight. Empirically, the SNR-collapse threshold sits at adapter-to-base ratios ≈2⁻⁴ of the per-block scale; the huikang AIMO2 adapter we tested had per-element delta ratios ≈2⁻⁶ of base scale, well inside the collapsed regime (Fig. 2). Leaderboard impact: 42 → 40.
- **EAGLE-3 draft-target speculative decoding [15] producing zero-or-degenerate output tokens on vLLM 0.11.2 + MXFP4 + sparse-MoE** (§6.1, `tests/test_eagle3_moe_zero_tokens.py`) — the EAGLE-3 auxiliary draft head assumes dense-layer hidden states; MoE expert dispatch in gpt-oss-120b breaks feature alignment, and the MXFP4 kernel path in vLLM 0.11.2 lacks the fused speculative-verify kernel. The failure surface upstream is heterogeneous: load-time `NotImplementedError`, severe quality collapse, or empty-completion fallback; on our exact substrate the manifestation was empty completions silently parsed to a zero answer. Tracked upstream in vllm-project/vllm (issue IDs in §6.1). Leaderboard impact: 42 → 0.
...
**Artifact 3: A reproduction specification and one-command script** (§3) that regenerates our final `submission.parquet` to within two-run seed tolerance on any H100 80GB. The specification pins the vLLM wheel SHA, the gpt-oss-120b weight commit, the Kaggle Docker image digest, a complete `pip freeze`, all prompts verbatim, and the decoding configuration. Our final submission scored 42/50 public and 39.5/50 private — a single realization of the stochastic-sampling distribution analysed in §2.7, with the −2.5 pp gap sitting 0.4σ inside the variance-model band (σ ≈ ±6.3 pp on a paired 50-problem gap; 95% interval ≈ ±9 pp). The observation is consistent with the variance model but not evidence of it; we offer the package as an end-to-end reproducibility target, not as evidence of a substrate ceiling.

We **explicitly do not claim** 42 / 50 is the achievable ceiling on this substrate; the released private leaderboard places the top four teams at 44.0/50 (≈+4.5 pp above our private 39.5), and their methods are not publicly disclosed at the time of writing. What we document is a **practitioner's plateau**: the failure envelope of a two-person, inference-only effort that had full access to the public-consensus recipe and 22 days of H100 time to depart from it. We extrapolate tentatively, grounded in the training-heavy wins of Numina [3] and NemoSkills [4], that the path beyond the 44.0 cluster on this substrate likely requires training-data interventions which the quantization regime forecloses without access to full-precision weights.
...
*Figure 3. Submission timeline over the 22-day study. The day-5 baseline (v7 Winner Fork, 42/50 public) is never exceeded; all 16 subsequent departures regress or are leaderboard-neutral. The final submission (day 22) is byte-identical to the day-5 baseline and scored 42/50 public, 39.5/50 private — a −2.5 pp gap consistent with the variance band of §2.7. (Plot is on the public-LB axis; post-deadline private scores were released 2026-05 and are discussed in §3.6.)*
...
Our answer, below, is that **none of the sixteen departures we tested did**. The best-performing configuration we found is the unmodified baseline. This is not evidence of a true substrate ceiling; it is evidence of a **practitioner's plateau** — the failure envelope of our own 22-day inference-only effort. Training-data interventions (foreclosed for us by the quantization regime, per §6.2) remain a plausible and likely path to higher scores, consistent with Numina and NemoSkills. *Post-deadline corroboration:* the public top scorer ([Seungjun Lee, 45/50 public](https://www.kaggle.com/code/seungjunlee00/aimo3-public-45-50-public-solution-write-up)) used no fine-tuning, no EAGLE-3, and no Bayesian re-weighting — independently consistent with our trap diagnoses in §6.
...
We follow the negative-results reporting posture of Rosenfeld et al. [16] and Lipton & Steinhardt [17]. Each hypothesis is a controlled variant relative to the baseline, tested first on a 199-problem internal validation set (Appendix A3) and, where the validation signal warranted, on the leaderboard. The validation set has limited statistical power: for a McNemar paired test on 199 problems at baseline accuracy ≈0.80, the minimum detectable effect at α=0.05 and 1−β=0.80 is approximately 3–4 percentage points. Single-submission leaderboard observations on the 50-problem public split carry a binomial 95% CI of approximately ±6 points. All individual effect sizes reported must be read in this light: a "regression" under ±6 pp CI is statistically consistent with the null of no effect, not evidence of harm. The aggregate claim is stronger than any single component. Formally: under the two-sided null H₀: Δ_v = 0 vs H₁: Δ_v ≠ 0 (effect on validation), we observed zero BH-significant positives at q = 0.10 across the sixteen variants (per-variant McNemar p-values are two-sided), and the sixteen non-positive sign-test outcomes give a one-sided combined p ≈ 3 × 10⁻⁵ against the alternative "any tested inference-only departure improves over the baseline on this substrate". The validation-to-leaderboard transportability caveat still applies: the 199-problem validation distribution is not identical to the 50-problem public split; assumed discordant-pair rate for the MDE estimate is ~10–20%.
...
- This is **not a proof of an inference-time ceiling**. Four private-LB teams reached 44.0; the failure envelope we document is ours, not the substrate's.
...
We sample stochastically by design. For a problem solved with probability `p ∈ (0, 1)`, expected per-problem score is `p` under *both* deterministic-fixed-seed and stochastic-independent-seed strategies: `p² + 2p(1−p)·0.5 + (1−p)²·0 = p`. Time-based seeds are therefore not an EV play — they are a variance-reduction play. Per-problem variance of the *final* score is `Var_stochastic = 0.5 · p(1−p)` vs `Var_single-run = p(1−p)`: stochastic sampling halves per-problem variance, and under independence across the 50 problems halves the variance of the sum, tightening the realized-score interval around its expectation.

*Post-deadline note (added 2026-05-13, rebuttal phase).* The released private leaderboard scored our submission at 39.5/50, a −2.5 pp gap to our 42/50 public. Under the variance model derived above, σ(gap) ≈ 6.3 pp on a paired 50-problem split (95% interval ≈ ±12 pp); the realized gap is 0.4σ inside that interval (two-tailed p ≈ 0.69 against H₀: zero true gap). The host's post-release diagnosis of broad public→private movement as "intrinsic model variation" is independently consistent with this prediction. We frame the gap as **consistent with** the §2.7 variance model — not as evidence of it; a single realization cannot discriminate between substrate-variance, problem-distribution drift, and operator-luck. The pre-registered variance theorem precedes any private observation (Git commit, 2026-04-08, predating the release by five weeks).
...
| vLLM | 0.11.2 from official `gpt-oss` wheel (pinned by `kernel_sources: andreasbis/aimo-3-utils` [20]; the public GPT-OSS-120B-with-tools baseline [21] is the inference scaffold our pipeline builds on) |
| Base model | `danielhanchen/gpt-oss-120b/Transformers/default/1` (Kaggle Models) / [`unsloth/gpt-oss-120b`](https://huggingface.co/unsloth/gpt-oss-120b) (HuggingFace mirror); MoE experts in MXFP4, attention/router in BF16 |
...
### 3.6 Public–private gap as a reproducibility target

Our final submission scored 42/50 public and 39.5/50 private — a gap of −2.5 percentage points. Under the variance model of §2.7, the standard deviation of the paired public–private gap on a 50-problem split with per-problem solve-probability `p ≈ 0.8` is `σ ≈ √(2 · 0.5 · p(1−p) / 50) ≈ 6.3 pp`, giving a 95 % gap interval of approximately ±12 pp; the realized −2.5 pp sits 0.4σ inside this interval (two-tailed p ≈ 0.69). We therefore frame the observation as **consistent with** the stochastic-sampling model, not as evidence of it: a single realization cannot discriminate substrate-variance from problem-distribution drift or operator-luck. We do **not** claim this consistency as evidence of a substrate ceiling, of superior calibration, or of low variance per se. We offer it as a concrete end-to-end verification target: any re-implementation of §§3.1–3.4 that scores within approximately ±9 pp of 42 on an independent 50-problem draw is consistent with having faithfully reproduced the pipeline.
...
| Public 44/50 notebook (parthenos et al.) | None (inference-only) | gpt-oss-120b-MXFP4 | None | Harmony + TIR + K=8 + entropy-weighted | 44/50 public (private not separately published) |
| Seungjun Lee (public top, post-deadline) [18] | None (inference-only) | gpt-oss-120b-MXFP4 | None | Harmony + TIR + K=N + voting (no FT, no EAGLE-3, no Bayesian) | **45/50 public, 44/50 private** |
| **Ours** | **None (inference-only)** | **gpt-oss-120b-MXFP4** | **None** | **Harmony + TIR + K=8 + entropy-weighted** | **42/50 public, 39.5/50 private** |
| Private top-4 (tied) | Unknown | Unknown (likely gpt-oss-120b-MXFP4) | Unknown | Unknown | **44.0/50 private** (Exalted Joseph, varianceofx, SKobayak, taku) |
...
Comparing our 42 public / 39.5 private against the 45 public / 44 private of Seungjun Lee [18] and the 44.0 private of the top-4 tied teams: the ≈2-point public gap and the ≈4.5-point private gap to the leaders are both inside the one-submission binomial CI on a 50-problem split (±6 pp at 95%), and the post-deadline finding that the leaders used **the same recipe family** (Harmony + TIR + K=N + voting, no fine-tuning, no EAGLE-3, no Bayesian re-weighting) suggests the gap is largely sampling variance and small-margin selection-rule differences rather than a categorical method difference. The fact that the **private top-4 cluster at exactly 44.0** with 27 more teams at 43.5 is consistent with the variance-saturation regime predicted by §2.7. Our LoRA falsification (§6.2) is the relevant negative result here: the training path on MXFP4 gpt-oss-120b is foreclosed by the quantization regime, removing the principal axis along which scores could break out of the 42–45 band.
...
|---|---|---|---|---|---|---|
| gpt-oss-120b (ours) | 120B (MoE, 5.1B active) | MXFP4 experts, BF16 attn | — | — | — | 42/50 public, 39.5/50 private |
...
The released private leaderboard tightens the picture: top-4 tied at 44.0 / 50, 27 more teams at 43.5, the public top-scorer (Seungjun Lee, 45/50 public) landing at 44 private, and our submission at 39.5 — all within ≈5 pp of one another. Each pair-wise gap sits inside the one-submission 95 % CI on a 50-problem split (±6 pp). This is the variance-saturation regime predicted by §2.7: with the field converged on the same MXFP4 substrate and the same Harmony + TIR + K=N + voting recipe family, the realized scores cluster tightly around a substrate-imposed ceiling and the public–private movement is dominated by intrinsic stochastic-sampling variance rather than method differences. Our §6.2 LoRA falsification is directly consistent with this picture: the training path on MXFP4 gpt-oss-120b is foreclosed without access to full-precision weights, removing the principal axis along which scores could break out of the 39–45 band.
...
**Upstream evidence.** The failure mode is documented across multiple `vllm-project/vllm` issues on gpt-oss + EAGLE-3 + MXFP4 (verified open/closed statuses at the time of writing):

- [**#26328**](https://github.com/vllm-project/vllm/issues/26328) — *EAGLE3 with gpt-oss-120b not working*: startup crash with `NotImplementedError: Mxfp4 linear layer is not implemented` when loading an EAGLE3 drafter against the MXFP4 base. This is the **direct MXFP4+EAGLE incompatibility** cited in our hypothesis.
- [**#30401**](https://github.com/vllm-project/vllm/issues/30401) — *EAGLE3 failure with gpt-oss 20b and 120b*: general EAGLE3 + gpt-oss failure report, H100.
- [**#27626**](https://github.com/vllm-project/vllm/issues/27626) — *gpt-oss-120b with EAGLE3 Speculative decoding, awful benchmarks*: with EAGLE3, MMLU collapses 0.73 → 0.28 and HumanEval hits 0.22 on vLLM 0.11.1rc3 — quality collapse rather than literal zero tokens, but functionally equivalent for AIMO3 scoring.
- [**#32802**](https://github.com/vllm-project/vllm/issues/32802) — *GPT-OSS 0% prefix cache hits with hybrid attention + EAGLE*: confirms EAGLE breaks gpt-oss hybrid-attention path.
- [**#37295**](https://github.com/vllm-project/vllm/issues/37295) — *gpt-oss-120b + P-EAGLE speculative decoding causes openai_harmony parse errors and severe chat latency regression* (open): Harmony parser errors + 3× throughput collapse when enabling EAGLE on gpt-oss-120b.

In honest summary: on our exact target substrate (vLLM 0.11.2 + MXFP4 expert projections + MoE gpt-oss-120b + EAGLE-3 drafter), the observed failure mode in our v15 submission was empty / degenerate completions that silently parsed to a zero answer under fallback extraction. Across the broader vLLM 0.11.x × gpt-oss × EAGLE-3 surface, the failure manifests as either a load-time `NotImplementedError` (#26328) or a severe quality collapse sufficient to produce a 0/50 leaderboard result (#27626, #30401). Our `tests/test_eagle3_moe_zero_tokens.py::test_vllm_engine_returns_tokens` (gated on `ENABLE_H100_INTEGRATION=1`) reproduces the configuration and asserts non-empty, non-degenerate output; on affected vLLM builds the test fails, detecting the trap before a submission slot is burned.
...
**Mechanism.** MXFP4 is the OCP Microscaling FP4 format [1]: per 32-element block, each element is stored with 4 bits (1 sign bit + 3 bits of mantissa/exponent encoding `E2M1`) plus a single shared E8M0 block-level scale. E2M1 encodes eight distinct representable values per sign (including zero), giving a small finite set of representable magnitudes per block. When we compute `W' = Q(W + BA)` for base weight `W`, LoRA factors `B` ∈ ℝ^(d×r), `A` ∈ ℝ^(r×k), and MXFP4 quantizer `Q`, the delta `BA` must have per-element magnitude exceeding roughly half the gap between adjacent representable values in the block to survive re-quantization. The adapter we tested had per-element delta-to-base magnitude ratios consistently below the inter-bin gap: our `tests/test_lora_mxfp4_collapse.py` measures the post-requantization signal-to-noise ratio and detects collapse when `SNR ≤ 1` (adapter signal at or below MXFP4 quantization noise). Under that criterion, the adapter collapses and `W' ≈ W` within quantization noise — no adapter learning survives the re-quantization. This is the AIMO3 manifestation of the general merge-then-quantize incompatibility addressed by QA-LoRA [14] (which jointly optimizes adapter and quantization rather than merging post-hoc).
...
**Mechanism.** The Condorcet Jury Theorem [11] (with heterogeneous-probability extensions [12]) states that when independent voters each have individual correctness probability `p_i > 0.5`, plain majority voting is asymptotically optimal and strictly dominates sub-linear weighting rules that compress the majority's advantage over a minority. We invoke CJT in its **conditionally-independent / Boland (1989) heterogeneous form** rather than the strict-independence form, since K=8 rollouts off a single base model share priors and prompts and are at most conditionally independent given the input. Empirically on val-199, rollout-pair answer agreement is ≈0.71, well below the conditional-independence bound that would invalidate the CJT argument. On the AIMO3 reference set, the baseline per-rollout answer-level accuracy is `p_i ≈ 0.75` (median across rollouts), comfortably inside the CJT-applicable regime. Under plain majority, a 5-vote cluster beats a 2-vote cluster by a factor of `5/2 = 2.5`. Under √-prior posterior, the weights become `√5 ≈ 2.24` and `√2 ≈ 1.41`, and the margin shrinks from 2.5× to `2.24/1.41 ≈ 1.58×`. The √-prior weakens the Condorcet majority's advantage by construction — the plurality cluster is favored by a smaller factor than its vote count warrants. When this shrunken margin is combined with entropy weighting `1/max(H, H_floor)` (§2.6), a low-entropy confidently-wrong minority can overtake: in our tested failure, a 6-correct-vs-2-wrong cluster at average entropies (1.0, 0.375) gives combined score `√6 / 1.0 = 2.45` for correct vs. `√2 / 0.375 = 3.77` for wrong — flipping the correct plurality to the wrong answer (verified by `tests/test_bayesian_sqrt_inversion.py`). The failure mode is deterministic given the input, not a variance artifact.
...
**Caveat on the entropy weighting itself.** We did *not* run plain unweighted majority as an explicit 17th catalog entry. §6.3 demonstrates that *further* re-weighting on top of the entropy-weighted base hurts, but it does not isolate the entropy-weighting contribution itself. Other contestants have reported that token-level entropy is imperfectly calibrated for trace-correctness; isolating the entropy term is a clean follow-up we did not run.
...
1. **The "plateau" is practitioner-scoped.** Our empirical failure envelope is 42/50 public and 39.5/50 private on the §1.2 substrate, for a two-person team limited to inference-time interventions over 22 days. The private top-4 reached 44.0; we have no evidence that our envelope is general.
...
- **Verifier-trained selectors over a large rollout pool.** Concurrent to our work, natnitarach [22] reports `pass@100 ≈ 49/50` for gpt-oss-120b on the AIMO3 private set, with a ≈7-point gap to the N=8 majority — i.e., the correct answer is in the candidate pool but is outvoted. Our §4 catalog falsifies seven *prompt-rule / arithmetic-rule* selection variants under BH correction, which is negative evidence against naive selection-rule innovation, but it does **not** foreclose a *trained* verifier over a labelled rollout corpus (an approach analogous to NemoSkills [4] applied to inference-only deployment). On this substrate, with training of the base model foreclosed by quantization, a trained verifier-selector is the most plausible remaining inference-time path to close the pass@100 gap, and we agree this is a stronger framing of the next-step than "training is the only path".
...
[18] Lee, S. *AIMO3 Public 45/50 — Public-Solution Write-Up.* Kaggle Notebook (post-deadline), 2026. https://www.kaggle.com/code/seungjunlee00/aimo3-public-45-50-public-solution-write-up.
[19] Frieder, S. *Write-Up Prize Information* (host announcement post; informal Kaggle Discussion 689703, 2026).
[20] Andreas, B. *AIMO 3 | Utils.* Kaggle Notebook (utility script `andreasbis/aimo-3-utils`), 2026. https://www.kaggle.com/code/andreasbis/aimo-3-utils.
[21] Andreas, B. *AIMO 3 | GPT-OSS-120B (with tools).* Kaggle Notebook (inference baseline scaffold), 2026. https://www.kaggle.com/code/andreasbis/aimo-3-gpt-oss-120b-with-tools.
[22] Nitarach, N. *AIMO-3: Model Capability Dominate.* Kaggle Writeup (Write-Up Prize submission, post-deadline), 2026. https://www.kaggle.com/competitions/ai-mathematical-olympiad-progress-prize-3/writeups/aimo-3-model-capability-dominate; companion arXiv:2603.27844.
