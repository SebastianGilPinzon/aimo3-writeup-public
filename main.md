...
Our final submission's public and private leaderboard scores coincide at 42/50. Under the variance-reduction argument of §2.7 (`Var_stochastic = 0.5 · p(1−p)`), the single-realization probability of a faithful reproduction observing public–private parity depends on the problem-level solve-probability distribution, not just a gap mean/SD — we therefore decline to quote a single probability and instead frame parity as **consistent with** the stochastic-sampling model, not as evidence of it. We do **not** claim this parity as evidence of a substrate ceiling, of superior calibration, or of low variance per se. We offer it as a concrete end-to-end verification target: any re-implementation of §§3.1–3.4 that scores within approximately ±3 pp of 42 on an independent 50-problem draw is consistent with having faithfully reproduced the pipeline.
...
[18] Frieder, S. *Write-Up Prize Information* (host announcement post; informal Kaggle Discussion 689703, 2026).
